export default async function Home() {
  ...
  try {
    ...
    trending = data?.data?.songs || data?.songs || data?.results || data?.tracks || [];
  } catch (e) { trending = []; }

  const sections = [
    { title: 'Trending Tamil Albums', endpoint: 'trending?lang=Tamil', data: trending },
    { title: 'New Tamil Releases', endpoint: 'new-releases?lang=Tamil', data: [] },
    { title: 'Tamil Top Charts', endpoint: 'charts', data: [] },
  ];

  return (
    <main className="max-w-7xl mx-auto px-6 py-8 space-y-14">
      <section className="rounded-3xl overflow-hidden relative h-96 bg-gradient-to-tr from-rose-deep to-neutral-900 flex items-center px-12 shadow-2xl shadow-green-900/20">
      ...
      {sections.map((s) => (
        <section key={s.title}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {(Array.isArray(s.data) ? s.data.slice(0,6) : []).map((item, i) => (
              <a key={i} href={`/song/${item.id || i}`} className="group block">
                <div className="aspect-square rounded-2xl bg-neutral-900 overflow-hidden mb-3 shadow-lg shadow-green-900/20 group-hover:scale-[1.02] transition-transform">
                  <img src={item.image || item.albumArt || item.thumbnail || 'https://picsum.photos/seed/music/400'} alt={item.title || item.name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100" />
                </div>
