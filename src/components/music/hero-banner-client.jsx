    ...
    async function fetchFeatured() {
      try {
        const res = await fetch(
          `${JIOSAAVN_API}/search/songs?query=${encodeURIComponent("tamil trending 2025")}&limit=5`
        );
  ...
  return (
    <section className="relative px-4 md:px-8 pt-6 pb-2">
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-accent/20 via-bg-card to-gold/10 border border-border p-6 md:p-10">
        ...
        {image && (
          <div
            className="absolute inset-0 opacity-15 blur-3xl transition-opacity duration-1000"
            style={{
        ...
        <div className="relative flex items-center gap-6 md:gap-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={handlePlay}
                className="px-6 py-2.5 bg-accent hover:bg-accent-light text-white font-semibold rounded-full transition-all hover:scale-105 active:scale-95 shadow-lg text-sm flex items-center gap-2"
              >
